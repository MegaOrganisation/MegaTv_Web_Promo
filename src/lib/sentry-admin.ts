...
export async function fetchSentrySummary(): Promise<SentrySummary> {
  ...
  const projects = [process.env.SENTRY_PROJECT_ANDROID, process.env.SENTRY_PROJECT_WEB || "megacompagnon-web"].filter(Boolean) as string[];
