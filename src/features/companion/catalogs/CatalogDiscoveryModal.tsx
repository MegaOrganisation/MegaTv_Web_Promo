...
export function CatalogDiscoveryModal({ open, existingUrls, onClose, onConfirmAdd }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="catalog-discovery-title">
      <div className="mega-glass flex max-h-[min(90vh,760px)] w-full max-w-3xl flex-col overflow-hidden rounded-[28px] border border-[var(--mega-border)]">
        <div className="flex items-start justify-between gap-3 border-b border-[var(--mega-border)] px-5 py-4">
          <div>
            <h2 id="catalog-discovery-title" className="text-xl font-bold text-[var(--mega-text)]">
              Ajouter un catalogue
            </h2>
            <p className="mt-1 text-sm text-[var(--mega-text-muted)]">Recherche Trakt ou MDBList, comme dans l&apos;app MegaTv.</p>
          </div>
          <button type="button" onClick={onClose} className="focus-ring rounded-full p-2 text-[var(--mega-text-muted)] hover:bg-[var(--mega-card-bg)]">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="border-b border-[var(--mega-border)] px-5 py-4">
          <div className="flex gap-2">
            ...
            />
            <MegaButton type="button" onClick={search} disabled={loading}>
              ...
              {results.map((result) => {
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      "focus-ring flex w-full gap-3 rounded-[22px] border p-3 text-left transition",
                      alreadyAdded
                  ...
                  >
                    ...
                    <span className="self-center text-xs font-semibold text-[var(--mega-text-muted)]">{alreadyAdded ? "Ajouté" : "Choisir"}</span>
                  </button>
                );
              })}
            </div>
      ...
      {pending ? (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/55 p-4">
          <div className="mega-glass w-full max-w-md rounded-[24px] border border-[var(--mega-border)] p-5">
