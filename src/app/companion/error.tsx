...
export default function CompanionError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[var(--mega-background-deep)] px-4">
      <div className="mega-glass w-full max-w-lg rounded-[28px] border border-[var(--mega-border)] p-6 text-center">
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-[var(--mega-text-faint)]">MegaCompagnon</p>
        <h1 className="mt-3 text-2xl font-black text-[var(--mega-text)]">Page indisponible</h1>
        <p className="mt-3 text-sm leading-6 text-[var(--mega-text-muted)]">Le chargement a échoué. L&apos;erreur a été remontée à Sentry.</p>
