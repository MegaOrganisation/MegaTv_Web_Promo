...
export default async function AdminPage() {
  await requireAdmin();
  const [dashboard, sentry] = await Promise.all([getAdminDashboardData(), fetchSentrySummary()]);
  ...
  return (
    <ResponsiveShell title="Vue Admin" subtitle="Agrégats d'infrastructure, analytics Companion et monitoring Sentry. Aucun accès brut cross-user n'est exposé à l'UI.">
      ...
      <AdminInfrastructurePanel overview={dashboard.overview as AdminOverview} />

      <section className="mt-6 grid gap-6 xl:grid-cols-[1fr_1fr]">
        <AdminSentryPanel summary={sentry} />
        <GlassCard as="section">
          <h2 className="text-2xl font-bold text-white">Top global</h2>
          <p className="mt-2 text-sm text-white/45">Contenus agrégés sur 30 jours.</p>
          <div className="mt-6">
