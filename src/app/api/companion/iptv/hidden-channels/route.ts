...
import { saveIptvHiddenChannelsForProfile } from "@/lib/iptv/queries";
...
export async function POST(request: Request) {
  ...
  if (!Array.isArray(body?.hiddenChannels)) {
    return NextResponse.json({ error: "hiddenChannels array required" }, { status: 400 });
  }
  ...
  const hiddenChannels = body.hiddenChannels
    .filter((v): v is string => typeof v === "string")
    .map((v) => v.trim())
    .filter(Boolean);

  const result = await saveIptvHiddenChannelsForProfile(profileId, hiddenChannels);
  if (!result.ok) {
