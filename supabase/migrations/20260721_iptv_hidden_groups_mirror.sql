-- INC-WEB-039 : mirror hiddenCategories → hiddenGroups (Android Live TV parity)
-- + one-shot backfill for existing profiles where hiddenGroups is empty/shorter.
...
-- Backfill Papa & Maman (+ any profile) where hiddenGroups lags hiddenCategories
update public.account_sync_state ass
set
  payload = (
    with profiles as (
      select key as profile_id, value as state
      from jsonb_each(coalesce(ass.payload::jsonb -> 'iptvByProfile', '{}'::jsonb))
    ),
    fixed as (
      select
        profile_id,
        case
          when jsonb_typeof(state -> 'hiddenCategories') = 'array'
               and jsonb_array_length(state -> 'hiddenCategories') > 0
               and (
                 jsonb_typeof(state -> 'hiddenGroups') is distinct from 'array'
                 or jsonb_array_length(coalesce(state -> 'hiddenGroups', '[]'::jsonb))
                    < jsonb_array_length(state -> 'hiddenCategories')
               )
          then state || jsonb_build_object('hiddenGroups', state -> 'hiddenCategories')
          else state
        end as state
      from profiles
    ),
    new_iptv as (
      select coalesce(jsonb_object_agg(profile_id, state), '{}'::jsonb) as iptv
      from fixed
    )
    select (
      ass.payload::jsonb
      || jsonb_build_object(
        'iptvByProfile', new_iptv.iptv,
        'updatedAt', greatest(
          coalesce((ass.payload::jsonb ->> 'updatedAt')::bigint, 0) + 1,
          (extract(epoch from clock_timestamp()) * 1000)::bigint
        )
      )
    )::text
  ...
  updated_at = now()
where ass.payload::jsonb ? 'iptvByProfile';
